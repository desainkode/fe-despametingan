        ...
        {latestNews.map((news, idx) => (
          ...
          >
            ...
            <div className="p-5 md:p-6">
              <h3 className="mb-3 line-clamp-2 font-[Georgia,serif] text-lg font-bold leading-tight text-[#0B281F] transition-colors group-hover:text-[#009966] md:text-xl">
                <Link href={`/berita/${news.slug}`}>
                  {news.title}
                </Link>
              </h3>
              
              <p className="mb-5 line-clamp-2 text-[13px] leading-relaxed text-[#0B281F]/50">
                {news.excerpt}
              </p>

              <div className="flex items-center justify-between border-t border-[#0B281F]/5 pt-5">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[12px] font-bold text-[#0B281F]">{news.author.name}</span>
                </div>
                
                <Link 
                  href={`/berita/${news.slug}`}
                  className="flex h-9 w-9 items-center justify-center rounded-full bg-[#0B281F]/5 text-[#0B281F] transition-all group-hover:bg-[#009966] group-hover:text-white"
                >
