    ...
    <article className="group flex flex-col overflow-hidden rounded-[32px] border border-[#0B281F]/5 bg-white shadow-[0_12px_40px_rgba(0,0,0,0.04)] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(11,40,31,0.08)] sm:rounded-[40px]">
      ...
      <div className="flex flex-1 flex-col p-6 sm:p-8">
        <div className="mb-3 flex flex-wrap items-center gap-4 text-[11px] text-[#0B281F]/50 sm:mb-4 sm:gap-4 sm:text-[12px]">
          ...
          <div className="flex items-center gap-1.5 font-medium">
            <Clock size={13} className="text-[#009966] sm:size-14" />
            {news.readingTime}
          </div>
        </div>

        <h3 className="mb-3 font-[Georgia,serif] text-xl font-bold leading-snug text-[#0B281F] transition-colors group-hover:text-[#009966] sm:mb-4 sm:text-2xl">
          <Link href={`/berita/${news.slug}`}>
            {news.title}
          </Link>
        </h3>

        <p className="mb-6 line-clamp-2 text-[13px] leading-relaxed text-[#0B281F]/60 sm:mb-8 sm:line-clamp-3 sm:text-[14px]">
          {news.excerpt}
        </p>
        ...
        <div className="mt-auto flex items-center justify-between border-t border-[#0B281F]/5 pt-4 sm:pt-6">
          <div className="flex items-center gap-2.5 sm:gap-3">
            ...
            <div>
              <p className="text-[12px] font-bold text-[#0B281F] sm:text-[13px] leading-tight">{news.author.name}</p>
              <p className="text-[10px] text-[#0B281F]/50 sm:text-[11px] mt-0.5">{news.author.role}</p>
            </div>
          </div>

          <Link
            href={`/berita/${news.slug}`}
            className="flex h-10 w-10 items-center justify-center rounded-full bg-[#0B281F]/5 text-[#0B281F] transition-all duration-300 group-hover:bg-[#009966] group-hover:text-white sm:h-11 sm:w-11"
          >
