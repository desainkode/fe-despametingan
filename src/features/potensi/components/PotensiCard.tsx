...
export function PotensiCard({ item }: PotensiCardProps) {
  return (
    <article className="group flex flex-col overflow-hidden rounded-[32px] border border-white bg-white shadow-[0_15px_45px_rgba(0,0,0,0.05)] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_25px_60px_rgba(11,40,31,0.1)] sm:rounded-[40px]">
      {/* Image Section */}
      <div className="relative h-48 w-full overflow-hidden sm:h-72">
        <img 
          src={item.image} 
          alt={item.name} 
      ...
      <div className="flex flex-1 flex-col p-6 sm:p-8 lg:p-10">
        <h3 className="mb-2 font-[Georgia,serif] text-xl font-bold leading-tight text-[#0B281F] transition-colors group-hover:text-[#009966] sm:mb-4 sm:text-2xl">
          {item.name}
        </h3>
        
        <p className="mb-6 line-clamp-2 text-[13px] leading-relaxed text-[#0B281F]/60 sm:mb-8 sm:line-clamp-3 sm:text-[15px]">
          {item.shortDesc}
        </p>

        {/* Highlights */}
        <div className="mb-6 grid grid-cols-2 gap-4 rounded-2xl bg-[#F6F8F7] p-4 sm:mb-10 sm:gap-4 sm:rounded-3xl sm:p-5">
          {item.highlights.map((hl, i) => (
            <div key={i} className="flex flex-col">
              <span className="text-[9px] font-extrabold uppercase tracking-widest text-[#0B281F]/30 sm:text-[11px]">{hl.label}</span>
              <span className="text-[13px] font-bold text-[#0B281F] sm:text-[16px]">{hl.value}</span>
            </div>
          ))}
        </div>
        ...
        <div className="mt-auto flex items-center justify-between border-t border-[#0B281F]/5 pt-4 sm:pt-8">
          <Link 
            ...
            className="flex items-center gap-1.5 text-[12px] font-bold text-[#0B281F] transition-colors hover:text-[#009966] sm:gap-2 sm:text-[14px]"
          >
          ...
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#0B281F] text-white shadow-lg shadow-[#0B281F]/20 transition-all group-hover:rotate-12 group-hover:bg-[#009966] sm:h-12 sm:w-12 sm:rounded-2xl">
            <Layers size={16} className="sm:size-20" />
          </div>
        </div>
      </div>
    </article>
